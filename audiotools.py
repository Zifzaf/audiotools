...
# Constants
third_octave_boundries = np.array([11.2, 14.1, 17.8, 22.4, 28.2, 35.5, 44.7,
                56.2, 70.8, 89.1, 112, 141, 178, 224, 282,
                355, 447, 562, 708, 891, 1122, 1413,
                1778, 2239, 2818, 3548, 4467, 5623,
                7079, 8913, 11220, 14130, 17780, 22390])
third_octave_center = np.array([12.5, 16, 20, 25, 31.5, 40, 50,
                63, 80, 100, 125, 160, 200, 250,
                315, 400, 500, 630, 800, 1000, 1250,
                1600, 2000, 2500, 3150, 4000, 5000, 6300,
                8000, 10000, 12500, 16000, 20000])
third_octave_names = ['12.5', '16', '20', '25',
...
def play_track(m, sr, device=sounddevice.default.device, channels=None, blocking=True):
    sounddevice.play(m, samplerate=sr, mapping=channels,blocking=blocking, device=device)

def rec_track(time, channels, sr=48000, device=sounddevice.default.device, blocking=True):
    m = sounddevice.rec(frames=int(time*sr), samplerate=sr, device=device,
            mapping=channels, dtype=np.double, blocking=blocking)
    return m, sr

def play_rec_track(m, sr, channels_out, channels_in, device=sounddevice.default.device, blocking=True):
    n = sounddevice.playrec(m, samplerate=sr, device=device, input_mapping=channels_in,
            output_mapping=channels_out, blocking=blocking)
    return n, sr

def load_track(path):
    m, sr = librosa.load(path, sr=48000, mono=True, dtype=np.double, res_type='kaiser_best')
...
def calc_xth_octave_band_cuts(x: np.double, fmin: np.double, fmax: np.double) -> np.ndarray:
